...
LABEL_TO_ACTIVITY_ID = {
    ...
    "Surface_Facilities_To_Be_Deployed":34,
    "Studies_To_Be_Carried_Out":35,

    "Work_Over_Operations":18,
    "Well_Testing":19,
    "BHP_Survey":20,
    "Slick_Line_Services":21,
    "Steaming_Services":22,
    "CTUoperations": 23,
...
def migrate_sections(cursor, refid_to_qprid):
    cursor.execute("""
        SELECT REFID, SEQ_NO, WORK_PROG_QUT_ACT, PLANNED_QUT_ACT, ACTUAL_QUT_ACT,
               ...
               a.CREATED_BY, ACTIVE, LABEL, TYPE
    ...
    for row in rows:
        (
            ...
            created_by, active, label, sec_type
        ) = row
        ...
        # Special condition: Exploration type with SEQ_NO > 18
        if sec_type == "EXPLORATION" and seq_no > 18:
            other_activity = label.replace("_", " ") if label else None
            cursor.execute("""
                INSERT INTO upstream_data_management.t_quarterly_report_exploration_activities (
                    quarterly_report_id, activity_id, other_activity,
                ...
                VALUES (%s, NULL, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 1)
            """, (
                qpr_id, other_activity, wp_act_fy, qtr_plan_act, qtr_actual_act, var_act, cum_act, remarks_act,
                wp_exp_fy, qtr_plan_exp, qtr_actual_exp, var_exp, cum_exp, percent_ach,
                user_id, active
            ))
            print(f"✅ Inserted EXTRA Exploration Activity: REFID {refid} → Other Activity '{other_activity}'")
            continue

        # Normal flow using mapping
